...
class DirtyDataExtractor:
    ...
    def _process_file_list(self, file_list_to_process: list) -> list[dict]:
        ...
        for file in file_list_to_process:
            ...
            user_entries = self._get_beautiful_soup(file_name=file)

            for user_entry in user_entries:

                one_game_reviews = self._get_ratings_from_user(user_entry)
    ...
    def _get_beautiful_soup(self, file_name: str) -> list[BeautifulSoup]:
        ...
        user_entries = game_page.find_all("item")

        print(f"\nTotal number of game entries in file: {len(user_entries)}\n")

        return user_entries

    def _get_ratings_from_user(self, user_entry: BeautifulSoup) -> list[list]:
        """Parse the individual game data"""
        print(user_entry)
        game_id = user_entry["id"]
        comments = user_entry.find_all("comment")
